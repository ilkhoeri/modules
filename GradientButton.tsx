...
import { AnchorTargetsProps } from './types';
...
export type hand_AnchorProps = AnchorTargetsProps &
...
export const GradientButton = (props: GradientButtonAllProps) => {
  ...
  if (core === 'anchor') {
    ...
    return (
      <GradientWrapper>
        ...
        >
          {children ??
            (label && <span className="truncate inline-block font-medium">{label}</span>)}
        </Link>
      </GradientWrapper>
    );
  }
  ...
  if (!core) {
    throw new Error(
      "Prop 'core' is required for GradientButton core. You Must Define GradientButton core!"
    );
  }
  return core;
};
